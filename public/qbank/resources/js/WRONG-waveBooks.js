...
// CheckValidity 
const checkValidityArabic = message =>{
    if (message === true) {
        const items =  [
            'https://books.knowledgewave.org/resources/audio/right-1.mp3' ,
            'https://books.knowledgewave.org/resources/audio/right-3.mp3',
            'https://books.knowledgewave.org/resources/audio/right-4.mp3',
            ];
            let randomNumb = Math.floor(Math.random() * 3);
            var item = items[randomNumb];
            const itemsMessage =  [
        ...
        document.getElementById('validate-message').style.color = '#c0392b' ;
        document.getElementById('valid-message-text').textContent = 'حاول مرة أخرى';
        document.getElementById('valid-message-text').style.fontSize = '3vw';
        document.getElementById('validate-message').style.borderColor = '#c0392b' ;
        document.getElementById('like-icon').classList = 'fas fa-thumbs-down' ;
        $("#validate-message").animate({left: '10px'});
        var audio = new Audio('https://books.knowledgewave.org/resources/audio/wrong-2.mp3');
        audio.play()   
        setTimeout(() => {
            $("#validate-message").animate({left: '-100%'});
        }, 1500);
...
const initValidation = ( className ) =>{
    document.querySelectorAll(className).forEach(elem=>(
        elem.addEventListener('input',
        ()=> elem.value === elem.getAttribute('validValue') ? checkValidityArabic(true) : checkValidityArabic(false))
    ))
...
const initChoose = ( className ) =>{
    document.querySelectorAll(className).forEach(elem=>(
        elem.addEventListener('click',
        ()=> elem.getAttribute('validValue') == 'true' ? checkValidityArabic(true) : checkValidityArabic(false))
    ))
